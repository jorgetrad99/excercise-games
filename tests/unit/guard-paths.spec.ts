...
describe('guard-paths hook', () => {
  ...
  it.each([
    ...
    ['worktree launcher', join(wt, '.claude', 'hooks', 'run-main.mjs'), 2],
    ['main root guard hook', join(main, '.claude', 'hooks', 'guard-paths.mjs'), 2],
    ['worktree settings.local.json', join(wt, '.claude', 'settings.local.json'), 2],
    ['worktree .mcp.json', join(wt, '.mcp.json'), 2],
    ['worktree CLAUDE.local.md', join(wt, 'CLAUDE.local.md'), 2],
    ['worktree AGENTS.md', join(wt, 'AGENTS.md'), 2],
    ['worktree CLAUDE.md (watched, not guarded)', join(wt, 'CLAUDE.md'), 0],
    ['worktree command (watched, not guarded)', join(wt, '.claude', 'commands', 'verify.md'), 0],
