...
function updateFloor(s: Readonly<BoxingState>, who: BoxerId, out: Presentation, fallen: number) {
  out.floor = fallen < 0 ? 0 : smooth(fallen / C.knockdown.fallS);
  out.stage = out.floor > 0 ? (out.floor < 1 ? 'fall' : 'down') : 'standing';
  if (s.down?.boxer !== who) return;
  // Back on the feet exactly when the sim resumes the fight (count = getUpAt): the sim's own timing.
  const k = smooth(riseProgress(s) ?? 0);
  if (k > 0) {
    out.floor *= 1 - k;
    out.stage = 'rise';
  }
