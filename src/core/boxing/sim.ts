...
export function tickBoxing(s: BoxingState, events: readonly BoxingInput[]): void {
  ...
  for (const b of s.boxers) stepBody(b, dt, s.phase === 'paused');
  switch (s.phase) {
    case 'intro':
    case 'break':
      s.phaseT -= dt;
...
/** Get-up blend progress 0…1 for the boxer down (null = not rising yet, or a KO count). It ends on the
 *  tick the count reaches getUpAt and the fight resumes. Shared by authority and render (PLAN-BOXING S6). */
export function riseProgress(s: Readonly<BoxingState>): number | null {
  if (s.phase !== 'down' && s.pausedFrom !== 'down') return null;
  if (!s.down || s.down.getUpAt >= 10) return null;
  const k = C.knockdown;
  const riseEnd = s.down.getUpAt * k.countS;
  const riseS = Math.max(0.1, Math.min(k.riseS, riseEnd - k.fallS));
  const into = s.phaseT - (riseEnd - riseS);
  return into >= -EPS ? Math.min(1, into / riseS) : null;
}
