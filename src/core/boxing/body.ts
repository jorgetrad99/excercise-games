...
function puppetGlove(b: Boxer, fist: Fist, hand: 0 | 1): V3 {
  ...
  // Hooks swing wide on the way; uppercuts dip under a guard and rise into the chin.
  out[0] += (end[0] - out[0]) * k + side * Math.abs(aim.x) * 0.35 * arc;
  out[1] += (end[1] - out[1]) * k - up * 0.22 * arc;
  out[2] += (end[2] - out[2]) * k;
  return out;
}
 ...
 */
export function stepBody(b: Boxer, dt: number): void {
  const track = b.body;
