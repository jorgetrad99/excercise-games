...
- **Drawn ahead, scored on what was seen (deliberately asymmetric).**
  ...
  - **BX-PRED-1 (proposed, not implemented; Jorge approves after the playtest): cap the drawn lead distance at `body.maxLeadM` = 0.15 m.**
    ...
    - **What the data says:**
      ...
      - **The worst lie measured is an instant reversal at the peak** (19.7 cm vs 12.7 cm with no prediction). Real reversals decelerate (the min-jerk rows show no such spike), and a cap ≥ 10 cm doesn't reach it. If playtest shows a glove flicking past the face at the end of a punch, this is the case, and the cap is the backstop.
    - **Proposed value: 15 cm.** It bounds every overshoot at 15 cm, never engages on jabs or leans at either rate, and engages only at the very fastest frames, where it improves accuracy.
