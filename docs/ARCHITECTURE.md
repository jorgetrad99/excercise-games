...
- `render/boxing/presentation.ts` observes each boxer's `hits` list (where each clean hit touched: zone and time). It owns bounded left/right/chin bruises, hit age, zero-stamina fall timing and count recovery. Repeated split-screen draws are idempotent; a new sim or rewind resets damage. Blocks do not create bruises (mid-round regen was removed in Boxing Phase 0 B3; the pie refills only on rule events). Core scoring and phases are unchanged: zero stamina starts the visual fall, the next clean hit still starts the official count.
