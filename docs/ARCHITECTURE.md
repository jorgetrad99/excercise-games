...
- `pnpm verify` = `tsc --noEmit` → `eslint .` → `vitest run` → `playwright test --project=smoke --project=smoke-perf` (the `perf` describe blocks run after the rest, one worker: they measure a GPU the other pages would share).
