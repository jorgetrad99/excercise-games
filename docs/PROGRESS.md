...
## 2026-09-17 — Handover merged into `fix/boxing-phase0`; verified; `capture=b1` checked end to end

- **Committed** `feat/player-authority` `2eae3c9`: the architecture session's uncommitted handover (`APPLY_BODY_SCAN = false`, body-scan e2e pins scanned = unscanned, BX-PRED-1 approved at 15 cm in PLAN-BOXING, M7.15 back to in_progress). `docs/AGENTS-AMENDMENT.md` stays untracked there (Jorge's by-hand AGENTS pass).
- **Merged** into `fix/boxing-phase0` as `7b91c29`. One conflict, PROGRESS.md (both sides appended): the "Update after Jorge's review" block now sits inside the M7.15 entry, before the Phase 0 entries; that entry's stale "Next" line dropped.
